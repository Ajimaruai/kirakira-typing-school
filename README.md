...
採用済みの主人公基準マスターは、次の場所で管理します。

```text
design/reference/main-character-master-v1.png
```

この画像は、顔立ち、頭身、髪色、制服、靴、リボン、配色、線の雰囲気を揃えるための正式な基準マスターです。今後のbody、face、hair、outfit、shoes、accessoryは、必ずこの画像と同じ位置・体格・ポーズで制作します。各パーツを別々にゼロから作ってはいけません。

マスターは`704 × 1208 px`のRGBA PNGで、アルファチャンネルと完全透明な背景を持っています。四隅は透明で、黒背景の焼き込みはありません。非透明部分はおよそ`x=125〜584`、`y=100〜1098`の範囲に収まり、中心と足元の透明余白を含めて基準座標とします。レイヤー制作時に自動トリミングせず、この余白も維持してください。

初案の`design/reference/main-character-v1.png`は制作履歴として残しますが、本番レイヤーの位置合わせには使用しません。

基準マスターはアプリが直接読み込む本番素材ではないため、`public/assets/avatar/`へコピーしたり、`data/items.ts`から参照したりしません。

#### face / body v1の検査・採用結果

最初のbaseレイヤー原本は次の場所に保存します。

```text
design/reference/layers/face-v1.png
design/reference/layers/body-v1.png
```

両方とも`704 × 1208 px`のRGBA PNGで、四隅は透明、黒背景の焼き込みはありません。正式マスターへ重ね、次の位置を確認したうえで採用しています。

- `face-v1.png`: 両目と顔の中心が正式マスターと数px以内で一致
- `body-v1.png`: 首、両手、両脚の位置が正式マスターと概ね一致

原本のサイズと透明情報は次のコマンドで確認します。すべて合格すると終了コード`0`になります。

```bash
pnpm run check:avatar-layer-sources
```

公開用には透明情報と画質を保つlossless WebPへ変換し、次の場所へ配置しています。
...
`data/avatarBase.ts`はこの2ファイルを参照します。ただし、必須slotであるhairBack、hairFront、outfit、shoesはまだ完成していません。中途半端に画像とCSSを混ぜないため、アプリは従来どおり安全な`cssFallback`表示になります。

画像の実寸と透明情報は、追加ライブラリなしで次のコマンドから確認できます。

```bash
pnpm run check:avatar-reference
```

基準マスターが`704 × 1208 px`で透明情報を持っていれば、この検査は成功します。次のコマンドでは、現在必須のbodyとface、レイヤーの`704 × 1208 px`、サムネイルの`512 × 512 px`、透明情報をまとめて検査します。未完成のhair、outfit、shoes、accessory、thumbnailsは「未制作（現在は正常）」と表示し、失敗にはしません。追加済みの画像に仕様違反があれば失敗します。

```bash
pnpm run check:avatar-assets
```

このマスターはすでに標準サイズへ適合しているため、`main-character-master-v1-704x1208.png`のようなリサイズ版は作りません。同じ内容の複製を増やさず、正式マスター1枚を基準にします。
...
`design/reference/main-character-master-v1.png`を見た目と位置の基準として、最初の動作確認セットでは次のレイヤーを同時に揃えます。画像を機械的に切り分けたり、各パーツを別々に生成したりせず、同じマスターを参照して透明レイヤーとして制作します。`accessory-big-ribbon`という新しいIDは作らず、既存の`itemId`である`accessory-ribbon`をファイル名に使います。
...
最初から別の髪型や服を増やさず、この1セットだけでホーム、着せ替え、タイピング、クリア画面の位置と切り抜けを確認してから量産します。

faceとbodyは採用済みです。次は次の順番で制作します。
...
共通baseのbodyとfaceは正式WebPを設定済みです。`hair-bob`、`outfit-uniform`、`shoes-brown`、`accessory-none`にはまだテスト用の透明SVGが含まれ、特に`hairBack`を含む完成セットは揃っていません。そのため、見た目は従来のCSS版から変わりません。
...
3. bodyとfaceは採用済みなので、`public/assets/avatar/base/`と`data/avatarBase.ts`のパスを確認します。
